...
- 每个 seed 与候选链的 v2 JSON：`<primary>__<address>__vs__<secondary>.json`
...
每个 seed 都会在四条快照中召回候选。`scoped_duplicate_scale` 和 `scoped_paper_stats` 均按 primary/secondary chain 分桶；NFT/合约比例始终以 primary chain 的完整快照总数为分母。链矩阵行保留对应候选链的 `native_symbol`，跨链汇总不合并 ETH、MATIC、SOL 原生金额，只输出可加总的 USD 金额。任何 work unit 失败不会取消其他 seed；成功的 v2 scoped cache 会保留，进程最终返回非零，下一次运行只重试缺失或失败的链。
...
- `--max-history-transactions-per-collection 10000`：单个 Solana collection 最多进入交易详情阶段的签名引用总数；达到预算后保留截断质量标记。
- `--alchemy-network chain=network`：可重复指定 EVM 网络覆盖；未指定时自动使用对应主网。
- `--feature-parquet PATH`：可重复传入，文件内链字段决定导入链；正式四链分析要求 feature store 同时包含四条链。
