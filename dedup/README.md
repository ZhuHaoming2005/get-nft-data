...
候选评分和媒体下载采用生产者—消费者流水线：核心层继续随机搜索时，32 个独立下载 worker 在容量为 64 个 URI 的有界队列中并发工作，同一 NFT 对的 A/B 图片也并行下载。运行中相同 URI 的在途请求会合并；成功媒体按 URI 的 SHA-256 键写入输出目录下的 `.metadata-image-cache`，后续候选或后续运行通过硬链接（不可用时复制）复用原始字节，失败响应不缓存。Metadata profile、候选 posting 和查重索引只存在于本次进程内存中，不读取或写入磁盘缓存；`.metadata-image-cache` 是唯一的持久缓存，并且只用于避免重复媒体/API 请求，不参与候选选择或普通查重。
