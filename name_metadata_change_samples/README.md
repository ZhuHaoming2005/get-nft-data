...
- `Modification Summary`：按 name/metadata 分别统计修改方式标签，输出 `count (percent)`。`total matches` 是该侧参与统计的重复记录数；标签采用叠加统计，同一条重复记录可同时计入多个标签，因此各标签比例相加可能超过 100%；未命中任何规则时计入 `other`。seed name 为空、`None`、`null`、全问号等无效值时，该 seed 的 name matches 不参与 name 统计。
