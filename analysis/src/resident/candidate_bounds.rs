...
///
/// Shared between `resident::name_index` (safe-prefix construction) and
/// `dedup::name` (post-candidate overlap/length verification) so both sides
/// agree on exactly the same provable bounds.
...
impl CandidateBounds {
    pub fn minimum_multiset_overlap(
        ...
        let mut low = 0;
        let mut high = left_len.min(right_len).saturating_add(1);
        while low < high {
...
mod tests {
    ...
    fn production_bounds_cover_exhaustive_small_alphabet_hits() {
        ...
        for left in &values {
            for right in &values {
                let score =
                    rapidfuzz::distance::jaro_winkler::similarity(left.chars(), right.chars())
                        * 100.0;
                if score < 95.0 {
                    continue;
                }
