...
fn snapshot_fixture() -> (tempfile::TempDir, std::path::PathBuf, std::path::PathBuf) {
    ...
    commit_ready(
        &f,
        "features.ready",
        r#"{"schema_revision":1,"source_count":2,"payload_count":2,"chains":["x"],"chain_totals":[{"name":"x","contracts":2,"nfts":2}]}"#,
    )
    .unwrap();
    commit_ready(
        &b,
...
fn shared_token_local_routing_is_admitted_by_routed_work() {
    ...
    commit_ready(
        &features,
        "features.ready",
        r#"{"schema_revision":1,"source_count":300,"payload_count":300,"chains":["x"],"chain_totals":[{"name":"x","contracts":300,"nfts":300}]}"#,
    )
    .unwrap();
    commit_ready(
        &blocking,
        "blocking.ready",
        r#"{"blocking_revision":1,"atom_count":300}"#,
    )
    .unwrap();
...
fn shared_token_exact_parallelism_preserves_deterministic_evidence() {
    ...
    commit_ready(
        &blocking,
        "blocking.ready",
        r#"{"blocking_revision":1,"atom_count":4}"#,
    )
    .unwrap();
    let snapshot = MetadataSnapshot::open(&features, &blocking).unwrap();
...
fn recovered_connectivity_is_rejected_under_a_lower_candidate_cap() {
    ...
    commit_ready(
        &features,
        "features.ready",
        r#"{"schema_revision":1,"source_count":3,"payload_count":1,"chains":["x"],"chain_totals":[{"name":"x","contracts":3,"nfts":3}]}"#,
    )
    .unwrap();
    commit_ready(
        &blocking,
...
fn catalog_progress_counts_routing_pairs_not_conditional_contract_products() {
    ...
    commit_ready(
        &features,
        "features.ready",
        r#"{"schema_revision":1,"source_count":40,"payload_count":2,"chains":["x"],"chain_totals":[{"name":"x","contracts":40,"nfts":40}]}"#,
    )
    .unwrap();
    commit_ready(
        &blocking,
...
fn scope_forests_do_not_leak_transitive_edges_between_chain_pairs() {
    ...
    commit_ready(
        &features,
        "features.ready",
        r#"{"schema_revision":1,"source_count":3,"payload_count":1,"chains":["a","b","c"],"chain_totals":[{"name":"a","contracts":10,"nfts":100},{"name":"b","contracts":20,"nfts":200},{"name":"c","contracts":30,"nfts":300}]}"#,
    )
    .unwrap();
    commit_ready(
        &blocking,
...
fn summary_preserves_selected_chains_without_eligible_metadata_contracts() {
    ...
    commit_ready(
        &features,
        "features.ready",
        r#"{"schema_revision":1,"source_count":1,"payload_count":1,"chains":["a","b","c"],"chain_totals":[{"name":"a","contracts":1,"nfts":1},{"name":"b","contracts":20,"nfts":200},{"name":"c","contracts":30,"nfts":300}]}"#,
    )
    .unwrap();
    commit_ready(
        &blocking,
