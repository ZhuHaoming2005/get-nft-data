...
fn match_forecast_requires_eight_fresh_successes_with_an_exact_key() {
    ...
    fs::create_dir_all(artifacts.join("encode-1")).unwrap();
    fs::create_dir_all(artifacts.join("blocking-1")).unwrap();
    fs::write(
        artifacts.join("encode-1/features.ready"),
        br#"{"schema_revision":1,"source_count":1000,"payload_count":500,"chains":[],"chain_totals":[]}"#,
    )
    .unwrap();
    fs::write(
        artifacts.join("blocking-1/blocking.ready"),
        br#"{"blocking_revision":1,"atom_count":300}"#,
    )
    .unwrap();
...
fn failures_and_resume_recomputes_never_calibrate_fresh_match_eta() {
    ...
    fs::write(
        artifacts.join("encode-1/features.ready"),
        br#"{"schema_revision":1,"source_count":10,"payload_count":5,"chains":[],"chain_totals":[]}"#,
    )
    .unwrap();
    fs::write(
        artifacts.join("blocking-1/blocking.ready"),
        br#"{"blocking_revision":1,"atom_count":3}"#,
    )
    .unwrap();
    let key = match_observation_key(&manifest, temp.path()).unwrap();
...
fn match_scale_key_rejects_equal_counts_with_different_membership_density() {
    let temp = tempfile::tempdir().unwrap();
    let manifest = sample_manifest(temp.path());
    let encode = temp.path().join("artifacts/metadata/encode-1");
    let blocking = temp.path().join("artifacts/metadata/blocking-1");
...
fn match_scale_key_rejects_equal_bytes_with_different_candidate_pair_work() {
    ...
    fs::write(
        encode.join("features.ready"),
        br#"{
        "schema_revision":1,"source_count":1000,"payload_count":500,
    ...
    fs::write(
        blocking.join("blocking.ready"),
        br#"{
        "blocking_revision":1,"atom_count":300,
...
fn match_observation_history_survives_work_cleanup_and_names_sampled_resources() {
    ...
    let encode = work.join("artifacts/metadata/encode-1");
    let blocking = work.join("artifacts/metadata/blocking-1");
    fs::create_dir_all(&encode).unwrap();
    fs::create_dir_all(&blocking).unwrap();
    fs::write(
        encode.join("features.ready"),
