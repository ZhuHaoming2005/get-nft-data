...
- metadata 解析批次为 16K 条合约级代表行，metadata BM25 评分按 256 个 left-doc 一批调度；候选 right-doc 在 worker 内随生成随评分，并通过 scratch pool 复用候选去重数组；postings、候选列表和 metadata contract membership 使用紧凑 `u32` 编号；BM25 相似边会先经过 0.5% 文档频率的稀有锚点门控，适配全局重复比例实验并限制候选桥接误判。
