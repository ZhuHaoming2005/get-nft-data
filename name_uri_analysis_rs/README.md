...
- `metadata` 使用 BM25 文档查重，阈值为 `0.6`；每个合约保留可查重的唯一 metadata 文档，先并行解析 metadata 并按全局唯一文档去重，再并行构建倒排索引、BM25 query 和候选文档对评分，命中后在合约层合并重复组。metadata 评分会先精确统计候选文档对总数，进度条消息显示剩余候选对数和已命中文档对数。
...
- 默认面向 64 核、256GB 内存机器：`--threads` 默认 `64`，`--memory-limit` 默认 `auto`。
...
- metadata 解析批次为 16K 行，metadata BM25 评分按 2048 个 left-doc 一批调度，适配 64 核机器减少调度开销。
