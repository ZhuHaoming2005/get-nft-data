...
3. `RecallController`：在独立 tuning/holdout 样本上选择 `JointBaseEquivalent`、`JointBounded` 或受预算限制的 rescue。
...
- bounded profile 超过门限时只允许切换到更宽 profile、受预算 rescue 或明确失败，不能静默截断候选。
...
Exact 成本仅用于校准预算，不能复用为 Conservative 正式 ETA。进度分别报告 estimated/visited joint、anchor 和 exclusion visits。
...
- 任何 bucket/count/index 溢出、内存预检失败、校准证据不足或 rescue 超预算均返回明确错误。
